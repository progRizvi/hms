...
class SslCommerzPaymentController extends Controller
{
    ...
    public function index(Request $request, $id)
    {
        ...
        $post_data['total_amount'] = $request->advance;
        ...
        $validation = Validator::make($request->all(), [
            "check_in" => "date|after_or_equal:now",
            "check_out" => "date|after_or_equal:check_in",
        ]);
        ...
        foreach ($period as $key => $date) {
            foreach ($roomAvailability as $availableRoom) {
                ...
                if ($availableRoom = date('Y-m-d', strtotime($date))) {
                    toastr()->error('Opps !!', 'Room Not Available on this day');
                    return redirect()->route('available.rooms');
                }
            }
